...
def download_admitted_student(request):
    """Generate pdf."""
    # Model data

    get_event = AcademicYear.objects.all().order_by('id').first()
    get_student = Student.objects.filter(entry_date__year=get_event.year).order_by('entry_number')

    template_name = str(get_event.year) + "selected students"
    name = f"{template_name}.pdf"
    # Rendered
    html_string = render_to_string('SRS/admitted_student.html',
                                   {'get_student': get_student, 'year':get_event})
...
def parent_download_result(request, rank, combination):
    ...
    template_name = str(rank) + "_" + str(combination) + "_" + str(get_event.event) + "results" + "_" + str(
        get_event.year)
    name = f"{template_name}.pdf"
    ...
    # Rendered
    html_string = render_to_string('SRS/pdf.html',
                                   {'get_student': get_student, 'result': get_student_results, 'subject': get_subjects,
                                    'total': get_total,
